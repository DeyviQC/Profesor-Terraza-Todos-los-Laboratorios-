...
print("Los idiomas más hablados son:  \n")

def los_idiomas_mas_hablados_en_el_mundo():
    idiomas = {}
    ...
    return [idioma for idioma, _ in idiomas_ordenados[:20]]

print(los_idiomas_mas_hablados_en_el_mundo())
...
def los_paises_mas_poblados():
    paises = {}
    for country in countries:
        paises[country["name"]] = country["population"]

    paises_ordenados = sorted(paises.items(), key=lambda x: x[1], reverse=True)
    
    return [pais for pais, _ in paises_ordenados[:30]]

print(los_paises_mas_poblados())
